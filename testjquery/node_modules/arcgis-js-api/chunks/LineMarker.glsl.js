...
return abs(cos) > 0.001 && t > 0.0;
  ...
  `)),v.addPixelRatio(C),a.addCalculateLinearDepth(P),C.code.add(f.glsl`void main(void) {
if (uv0.y == 0.0) {
...
displacedPosScreen = projectAndScale(pos);`)),C.code.add(f.glsl`
  ...
  `),z&&P.include(l.multipassTerrainTest,e),P.include(t.SliceDraw,e),k.uniforms.add(new h.Float4PassUniform("intrinsicColor",(e=>e.color)),new S.Texture2DPassUniform("tex",(e=>e.markerTexture))),k.include(d.ColorConversion),P.constants.add("texelSize","float",1/r.markerTextureSize),k.code.add(f.glsl`float markerAlpha(vec2 samplePos) {
samplePos += vec2(0.5, -0.5) * texelSize;
float sdf = rgba2float(texture(tex, samplePos)) - 0.5;
float distance = sdf * vSize;
distance -= 0.5;
return clamp(0.5 - distance, 0.0, 1.0);
}`),e.hasTip&&(P.constants.add("relativeMarkerSize","float",r.markerSymbolSize/r.markerTextureSize),P.constants.add("relativeTipLineWidth","float",r.markerTipThicknessFactor),k.code.add(f.glsl`
  ...
  `)),P.constants.add("symbolAlphaCutoff","float",c.symbolAlphaCutoff),k.code.add(f.glsl`
  void main() {
    discardBySlice(vpos);
    ...
    ${e.output===i.ShaderOutput.Alpha?f.glsl`fragColor = vec4(finalColor.a);`:""}
    ${e.output===i.ShaderOutput.Color?f.glsl`fragColor = highlightSlice(finalColor, vpos);`:""}
    ${e.output===i.ShaderOutput.Color&&e.transparencyPassType===y.TransparencyPassType.Color?"fragColor = premultiplyAlpha(fragColor);":""}
    ${e.output===i.ShaderOutput.Highlight?f.glsl`fragColor = vec4(1.0);`:""}
